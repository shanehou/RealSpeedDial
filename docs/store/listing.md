...
- Core features need bookmarks, storage, favicon, context menu, activeTab, and scripting access only
...
**隐私优先的权限设计**
- 核心功能仅需书签、存储、favicon、右键菜单、activeTab 与 scripting 权限
...
Used when the user chooses a page context-menu item to capture the current visible page — either the full viewport or a user-drawn region — as a Speed Dial thumbnail. The menu click is a user gesture; activeTab grants temporary access to that tab (and, with scripting, to inject the region overlay) without permanently requesting broad host access for this manual flow.

**中文**

用户通过页面右键菜单截取当前可见页面（整页或框选区域）为缩略图时使用。该点击属于用户手势；activeTab 仅临时授权当前标签页（并与 scripting 配合注入框选遮罩），无需为此手动流程永久申请宽泛主机权限。
...
Adds browser context-menu commands on http(s) pages so the user can capture the current visible page — full viewport or a drawn region — as a thumbnail for a matching bookmark (or pick a bookmark if the URL is not bookmarked yet).

**中文**

在 http(s) 页面上添加浏览器右键菜单项，供用户将当前可见页面（整页或框选区域）截取为对应书签的缩略图；若当前 URL 尚未收藏，则打开书签选择器指定目标。
...
Used with `activeTab` when the user chooses the page context-menu item to capture a region as a Speed Dial thumbnail. Injects a short-lived, self-contained selection overlay into the current tab so the user can draw and fine-tune a focus region; no permanent host access is required for this flow.

**中文**

用户通过页面右键菜单「截取区域设为缩略图」时，与 `activeTab` 配合使用。向当前标签页注入一次性、自包含的框选遮罩，供用户拖拽并微调焦点区域；此流程无需永久主机权限。
