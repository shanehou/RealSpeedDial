...
- **磁贴样式**：带稳定 URL 渐变背景的「图标 + 标题」，或带可读文字层的网页截图。
- **截图控制**：访问已收藏页面时自动更新缩略图；也可通过浏览器页面右键菜单截取你喜欢的当前状态。当前 URL 未收藏时，会弹出支持标题和 URL 模糊搜索的书签选择器。
- **区域截图**：在任意网页右键选择「截取区域设为缩略图」，拖拽框选并可拖角/拖边微调、拖动移动、回车确认，磁贴会放大显示该区域。
...
| `storage` | 设置与导航状态 | 安装时 |
| `favicon` | 显示网站图标 | 安装时 |
| `contextMenus` + `activeTab` | 从浏览器页面右键菜单截取当前可见页面 | 安装时 |
| `scripting` | 按需向当前标签注入区域框选遮罩（配合 `activeTab`，不触发主机权限弹窗） | 安装时 |
